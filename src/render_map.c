...
/*   Updated: 2025/02/15 18:19:01 by rcosta-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_wall(t_cub *cub, t_ray ray int x, int ds, int de, t_tex *tex, int lh, int texX)
{
    int y, d, texY, color;
    y = ds;
    while (y < de)
...
void render_column(t_cub *cub, int x)
{
    ...
    compute_ray_parameters(ray.cub, ray.x, &mx, &my, &rd_x, &rd_y, &step_x, &step_y,
                           &sd_x, &sd_y, &ddx, &ddy);
